...
/// Arc-based version: uses &[Arc<TickArrayState>] to avoid cloning TickArrayState.
/// Only reads from tick arrays, never mutates.
pub fn get_out_put_amount_and_remaining_accounts_with_arc(
    input_amount: u64,
    sqrt_price_limit_x64: Option<u128>,
    zero_for_one: bool,
    is_base_input: bool,
    ...
    pool_state: &PoolState,
    tickarray_bitmap_extension: &TickArrayBitmapExtension,
    tick_arrays: &[Arc<TickArrayState>],
) -> AnyResult<(u64, VecDeque<i32>)> {
    let (is_pool_current_tick_array, current_valid_tick_array_start_index) = pool_state
        .get_first_initialized_tick_array(&mut Some(*tickarray_bitmap_extension), zero_for_one)?;

    let (amount_calculated, tick_array_start_index_vec) = swap_compute_with_arc(
        zero_for_one,
        is_base_input,
        is_pool_current_tick_array,
        pool_config.trade_fee_rate,
...
fn swap_compute_with_arc(
    ...
    let tick_array_current = &tick_arrays[0];
    if tick_array_current.start_tick_index != current_valid_tick_array_start_index {
        return Result::Err(anyhow!("tick array start tick index does not match"));
    }
    let mut tick_array_start_index_vec = VecDeque::new();
        ...
        && state.tick > tick_math::MIN_TICK
    {
        ...
        let current_array = &tick_arrays[array_idx];
        let mut next_initialized_tick = if let Some(tick_state) = current_array
            .next_initialized_tick_ref(state.tick, pool_state.tick_spacing, zero_for_one)?
        {
            *tick_state
        } else {
            if !tick_match_current_tick_array {
                tick_match_current_tick_array = true;
                *current_array.first_initialized_tick_ref(zero_for_one)?
            } else {
                TickState::default()
            }
        };
        ...
        if !next_initialized_tick.is_initialized() {
            ...
            let Some(start_index) = next_start_index else {
                return Result::Err(anyhow!("tick array start tick index out of range limit"));
            };
            if next_array.start_tick_index != start_index {
                return Result::Err(anyhow!("tick array start tick index does not match"));
            }
            tick_array_start_index_vec.push_back(next_array.start_tick_index);
            next_initialized_tick = *next_array.first_initialized_tick_ref(zero_for_one)?;
        }
        step.tick_next = next_initialized_tick.tick;
        step.initialized = next_initialized_tick.is_initialized();
        if step.tick_next < MIN_TICK {
